...
class macro = object (m)
    ...
    method expression x =
      ...
      match x with
        | J.ECall(J.EVar (J.S {J.name="BLOCK"}), tag::args,_) ->
        ...
        | J.ECall(J.EVar (J.S {J.name="ISBLOCK"}), [e],_) ->
          J.EBin(J.EqEq, J.EUn (J.Typeof, J.EDot(m#expression e,"tag")), J.EStr ("number",`Utf8))
