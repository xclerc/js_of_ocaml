...
module Ctx = struct
  ...
  let initial blocks live mutated_vars share debug =
    { constr = Hashtbl.create 17; blocks; live; mutated_vars; share; debug }
...
let rec constant_rec ~ctx x level instrs =
  ...
  | Float_array a ->
      J.EArr (Array.to_list (Array.map (fun f -> Some (float_const f)) a)),
      instrs
  | Int64 i ->
      J.EArr [Some (int 255);
  ...
  | Prim (p, l) ->
    let res = match p, l with
      ...
      | Array_get, [x; y] ->
        let ((px, cx), queue) = access_queue' ~ctx queue x in
        let ((py, cy), queue) = access_queue' ~ctx queue y in
        (J.EAccess (cx, cy), or_p mutable_p (or_p px py), queue)
...
and translate_closures ctx expr_queue l loc =
  match l with
    [] ->
      ([], expr_queue)
      ...
      let (st, expr_queue) =
        ...
        | Set_field (x, n, y) ->
            ...
            flush_queue expr_queue mutator_p
              [J.Expression_statement
                  (J.EBin (J.Eq, access_field cx n, cy)), loc]
        (* | Offset_ref (x, 1) -> *)
        (*   (\* FIX: may overflow.. *\) *)
        ...
        | Offset_ref (x, n) ->
          (* FIX: may overflow.. *)
            ...
            flush_queue expr_queue mutator_p
              [J.Expression_statement
                  ((J.EBin (J.PlusEq, (access_field cx 0), int n))),
               loc]
        | Array_set (x, y, z) ->
            let ((_px, cx), expr_queue) = access_queue expr_queue x in
            let ((_py, cy), expr_queue) = access_queue expr_queue y in
